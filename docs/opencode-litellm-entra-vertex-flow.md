...
- 暴露两个 provider 的登录方法：
  `Entra LiteVertex` 和 `Entra LiteVertex - dev`
...
- 同时把固定 provider 配置安装到 `~/.config/opencode/opencode.json`
...
1. 用户运行 `opencode auth login`，或者在 TUI 的 `/connect` 里选择 `Entra LiteVertex` / `Entra LiteVertex - dev`
2. OpenCode 从全局配置里读取固定 provider 定义，不依赖当前项目环境变量
3. OpenCode 登录对话框显示 provider 名称 `Entra LiteVertex` 或 `Entra LiteVertex - dev`
4. plugin 通过 OpenCode 的 `oauth/auto` 视图返回 Microsoft Entra 的验证链接和 user code
5. 用户在任意有浏览器的设备上完成验证
6. Entra 返回 Access Token 和 Refresh Token
7. plugin 把 Refresh Token 安全地缓存在 `~/.config/opencode/entra-device-token.json` 或 `~/.config/opencode/entra-device-token.dev.json`
...
当前全局 plugin 负责的是 OpenCode 自己发出的 `litellm` 和 `litellm-dev` provider 请求，包括：
